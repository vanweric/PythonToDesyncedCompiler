...
const App = () => {
  ...
  const instruction_completions = Object.keys(desyncedExport["instructions"])
    .map((key) => {
      if (desyncedExport.instructions[key]["name"])
        return {
          label: formatStringAsFunction(
            desyncedExport.instructions[key]["name"],
          ),
  ...
  const constants_completions = ["values", "components", "items"]
    .flatMap((category) =>
  ...
  const runPythonCode = async () => {
    if (pyodide) {
      ...
      const result = pyodide.runPython(
        "python_to_desynced_pyodide(editortext)",
        { locals: locals },
      );
      ...
      else setOutput(translatedresult[1]);
  ...
  const loadCompiler = async () => {
    ...
    console.log(
      pyodide.runPython(await (await fetch("./desyncedcompiler.py")).text()),
    );
    console.log(
      pyodide.runPython("import_desynced_ops(path = None, jsonfile=test1)", {
        locals: locals,
      }),
    );
    setCompiling(false);
  ...
  useEffect(() => {
    const setupDScompiler = async () => {
      ...
      if (!loading) {
        ...
        console.log(
          pyodide.runPython(
            await (await fetch("./desyncedcompiler.py")).text(),
          ),
        );
        console.log(
          pyodide.runPython(
            "import_desynced_ops(path = None, jsonfile=test1)",
            { locals: locals },
          ),
        );
  ...
  const OutputAlert = () => (
    <Alert
      variant={compilationSuccess ? "success" : "danger"}
      style={{
        flexWrap: "wrap",
        wordWrap: "break-word",
        textAlign: "left",
    ...
    >
      <Alert.Heading>
        {compilationSuccess ? "Compiled Code:" : "Error:"}
      </Alert.Heading>
      {output.split("\n").map((line, index) => (
            ...
            <Navbar.Collapse id="basic-navbar-nav">
              <Nav className="me-auto">
                ...
                >
                  ...
                  <NavDropdown.Divider />
                  <NavDropdown.Item
                    href="https://github.com/vanweric/PythonToDesyncedCompiler/tree/main/public/examples"
                    target="_blank"
                  >
                    Examples Folder
                  </NavDropdown.Item>
